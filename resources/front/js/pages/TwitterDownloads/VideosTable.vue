    ...
    <div class="twitter_videos_table">
        ...
        >
            ...
            <template v-slot:item.timestamp="{ item }">
                <span>{{item.timestamp | format_date_from_timestamp}}</span>
            </template>

            <template v-slot:item.uploader="{ item }">
                <a class="link"
                   target="_blank"
                   :href="item.uploader_url">{{item.uploader | shorten(40)}}</a>
            </template>
    ...
    data() {
      return {
        headers: [
          ...
          {text: 'Download Links', value: 'data-table-expand', width: '15%', align: 'center'},
        ],
        videos: [],
        search: '',
      }
